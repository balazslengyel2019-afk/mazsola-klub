...
public final class SynthViewV2 extends View {
    ...
    private void layoutRects() {
        float left = w * .108f, right = w * .892f;
        float gap = w * .0125f;
        ...
        youtubeRect.set(w*.425f, h*.035f, w*.585f, h*.115f);
        websiteRect.set(w*.790f, h*.035f, w*.965f, h*.115f);

        float keyLeft = w*.092f, keyRight = w*.908f;
        float keyTop = h*.635f, keyBottom = h*.947f;
    ...
    private void drawSeasonBackground(Canvas c) {
        ...
        // gentle haze behind the controls so seasonal art stays visible without hurting readability
        p.setShader(new LinearGradient(0,h*.27f,0,h*.65f,Color.argb(0,255,255,255),Color.argb(70,255,247,185),Shader.TileMode.CLAMP));
        c.drawRect(0,h*.24f,w,h*.64f,p); p.setShader(null);
    }

    private void drawAmbientAnimation(Canvas c, float t) {
        // slow seamless clouds
        for (int i=0;i<6;i++) {
        ...
        drawBalloon(c,w*.725f + (float)Math.sin(t*.30f)*w*.010f,
                h*.120f + (float)Math.sin(t*.48f)*h*.010f,h*.082f);
    }
    ...
    private void drawSun(Canvas c,float cx,float cy,float r,float t) {
        ...
        for (int i=0;i<12;i++) {
            ...
            c.drawLine(cx+(float)Math.cos(a)*r*1.34f,cy+(float)Math.sin(a)*r*1.34f,
                    cx+(float)Math.cos(a)*r*1.72f,cy+(float)Math.sin(a)*r*1.72f,stroke);
        }
        c.restore();
    ...
    private void drawLogo(Canvas c) {
        if (logo == null) return;
        float size=h*.285f;
        RectF dst=new RectF(w*.018f,h*.012f,w*.018f+size,h*.012f+size);
        c.drawBitmap(logo,null,dst,p);
    }

    private void drawLinks(Canvas c) {
        drawBadge(c,youtubeRect,Color.rgb(226,36,43),"Dalaink",true);
        drawBadge(c,websiteRect,Color.rgb(40,132,222),"mazsolaklub.com",false);
    }

    private void drawBadge(Canvas c,RectF r,int color,String text,boolean playIcon) {
        p.setShadowLayer(h*.008f,0,h*.004f,Color.argb(70,0,0,0));
        setLayerType(View.LAYER_TYPE_SOFTWARE,p);
        p.setColor(Color.argb(245,255,255,255)); c.drawRoundRect(r,h*.035f,h*.035f,p);
        p.clearShadowLayer(); setLayerType(View.LAYER_TYPE_HARDWARE,null);
        RectF inner=new RectF(r.left+h*.007f,r.top+h*.007f,r.right-h*.007f,r.bottom-h*.007f);
        p.setColor(color); c.drawRoundRect(inner,h*.030f,h*.030f,p);
        float ix=inner.left+inner.height()*.52f, iy=inner.centerY();
        p.setColor(Color.WHITE);
        if(playIcon){Path tri=new Path();tri.moveTo(ix-inner.height()*.12f,iy-inner.height()*.18f);tri.lineTo(ix+inner.height()*.17f,iy);tri.lineTo(ix-inner.height()*.12f,iy+inner.height()*.18f);tri.close();c.drawPath(tri,p);} 
        else { stroke.setColor(Color.WHITE); stroke.setStrokeWidth(h*.0035f); c.drawCircle(ix,iy,inner.height()*.20f,stroke); c.drawOval(new RectF(ix-inner.height()*.10f,iy-inner.height()*.20f,ix+inner.height()*.10f,iy+inner.height()*.20f),stroke); c.drawLine(ix-inner.height()*.20f,iy,ix+inner.height()*.20f,iy,stroke); }
        p.setTypeface(Typeface.DEFAULT_BOLD); p.setTextSize(h*.030f); p.setTextAlign(Paint.Align.LEFT); p.setColor(Color.WHITE);
        float tx=inner.left+inner.height()*.94f; float ty=inner.centerY()-(p.ascent()+p.descent())/2f;
    ...
    private void drawInstrumentRack(Canvas c) {
        ...
        p.setStyle(Paint.Style.STROKE); p.setStrokeWidth(h*.006f); p.setColor(Color.rgb(249,187,28)); c.drawRoundRect(rack,h*.045f,h*.045f,p); p.setStyle(Paint.Style.FILL);

        for(int i=0;i<6;i++) {
            RectF r=instrumentRects[i];
            boolean selected=i==selectedInstrument;
            p.setColor(cardColors[i]); c.drawRoundRect(r,h*.030f,h*.030f,p);
            if(selected){stroke.setColor(Color.WHITE);stroke.setStrokeWidth(h*.006f);c.drawRoundRect(new RectF(r.left+h*.003f,r.top+h*.003f,r.right-h*.003f,r.bottom-h*.003f),h*.027f,h*.027f,stroke);} 
            Bitmap icon=icons[i];
            if(icon!=null){
                float maxW=r.width()*.66f,maxH=r.height()*.59f;
                float sc=Math.min(maxW/icon.getWidth(),maxH/icon.getHeight());
                float iw=icon.getWidth()*sc,ih=icon.getHeight()*sc;
                RectF d=new RectF(r.centerX()-iw/2,r.top+r.height()*.07f,r.centerX()+iw/2,r.top+r.height()*.07f+ih);
    ...
    private void drawKeyboard(Canvas c) {
        ...
        p.setColor(Color.rgb(255,200,31)); c.drawRoundRect(shell,h*.055f,h*.055f,p);
        p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(h*.007f);p.setColor(Color.rgb(244,168,15));c.drawRoundRect(shell,h*.055f,h*.055f,p);p.setStyle(Paint.Style.FILL);
        drawSpeaker(c,w*.055f,h*.80f,h*.037f); drawSpeaker(c,w*.945f,h*.80f,h*.037f);
        drawStar(c,w*.055f,h*.675f,h*.035f,Color.rgb(255,139,27)); drawStar(c,w*.945f,h*.675f,h*.032f,Color.rgb(34,157,242));
        ...
        for(int i=0;i<14;i++) {
            ...
            p.setColor(Color.rgb(250,250,247)); c.drawRoundRect(rr,0,0,p);
            float bandH=rr.height()*.23f;
            p.setColor(whiteColors[i]); c.drawRect(rr.left,rr.bottom-bandH,rr.right,rr.bottom,p);
            p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(Math.max(1,h*.002f));p.setColor(Color.rgb(176,176,176));c.drawRect(rr,p);p.setStyle(Paint.Style.FILL);
        }
        for(int i=0;i<10;i++) {
            RectF r=blackRects[i]; boolean down=blackPressCount[i]>0;
            float dy=down?h*.012f:0; RectF rr=new RectF(r.left,r.top+dy,r.right,r.bottom+dy*.20f);
            p.setShader(new LinearGradient(rr.left,0,rr.right,0,Color.rgb(30,33,38),Color.rgb(10,12,15),Shader.TileMode.MIRROR));
            c.drawRoundRect(rr,0,0,p);p.setShader(null);
        }
    }

    private void drawSpeaker(Canvas c,float cx,float cy,float r){p.setColor(Color.rgb(142,88,13));c.drawCircle(cx,cy,r,p);p.setColor(Color.rgb(255,220,69));c.drawCircle(cx,cy,r*.82f,p);stroke.setColor(Color.rgb(70,45,15));stroke.setStrokeWidth(r*.08f);for(int i=-3;i<=3;i++){float yy=cy+i*r*.18f;c.drawLine(cx-r*.58f,yy,cx+r*.58f,yy,stroke);}}
    private void drawStar(Canvas c,float cx,float cy,float r,int color){Path path=new Path();for(int i=0;i<10;i++){double a=-Math.PI/2+i*Math.PI/5;float rr=(i%2==0?r:r*.45f);float x=cx+(float)Math.cos(a)*rr,y=cy+(float)Math.sin(a)*rr;if(i==0)path.moveTo(x,y);else path.lineTo(x,y);}path.close();p.setColor(color);c.drawPath(path,p);}
    ...
    private void releaseKey(int pointerId) {
        ...
        if(pr.hit.black) blackPressCount[pr.hit.index]=Math.max(0,blackPressCount[pr.hit.index]-1);
        else whitePressCount[pr.hit.index]=Math.max(0,whitePressCount[pr.hit.index]-1);
    ...
    private void openUrl(String url) {
        try { Intent i=new Intent(Intent.ACTION_VIEW, Uri.parse(url)); getContext().startActivity(i); } catch(Throwable ignored) {}
    }

    @Override public boolean onTouchEvent(MotionEvent e) {
        int action=e.getActionMasked(); int ai=e.getActionIndex(); int pid=e.getPointerId(ai); float x=e.getX(ai),y=e.getY(ai);
        if(action==MotionEvent.ACTION_DOWN || action==MotionEvent.ACTION_POINTER_DOWN) {
            if(youtubeRect.contains(x,y)){openUrl("https://www.youtube.com/results?search_query=Mazsola+Klub");return true;}
        ...
        if(action==MotionEvent.ACTION_MOVE) {
            for(int i=0;i<e.getPointerCount();i++) moveKey(e.getPointerId(i),e.getX(i),e.getY(i)); return true;
        }
        if(action==MotionEvent.ACTION_UP || action==MotionEvent.ACTION_POINTER_UP || action==MotionEvent.ACTION_CANCEL) {
            if(action==MotionEvent.ACTION_CANCEL){for(int i=active.size()-1;i>=0;i--)releaseKey(active.keyAt(i));}
            else releaseKey(pid); return true;
        }
        return true;
    }
}
