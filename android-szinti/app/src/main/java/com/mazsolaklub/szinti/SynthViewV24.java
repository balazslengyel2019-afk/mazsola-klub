...
public final class SynthViewV24 extends View {
    ...
    // Child-friendly, transposed white-key sequences. 0 = first coloured white key.
    private static final int[][] SONGS = {
            {0,2,0,2,4,4, 0,2,0,2,4,4, 7,6,5,4,3,5,4, 7,6,5,4,3,5,4},
            {8,8,5,8,9,7,7,8,9,8,7,6,5,5,8,5, 6,6,3,6,7,5,5,6,7,6,5,4,3,3,6,3},
            {8,8,7,5,8,8,7, 8,8,7,5,8,8,7, 8,5,4,5,5,5, 6,6,5,3,6,6,5, 6,6,5,3,6,6,5, 6,3,2,3,3,3},
            {8,7,5,6,3,5, 8,7,5,6,3,5, 9,9,9,9,9,7, 8,8,8,8,8,7, 8,7,5,6,3,5, 9,9,8,8,7,7,6, 8,8,7,5,6,4,5},
            {2,5,5,4,3,3,2,2, 2,5,5,4,3,3,2,2, 2,3,4,5,5,4,3,4,5,5,4,3,2,2, 2,3,4,5,5,4,3,4,5,5,4,3,2,2}
    };
    ...
    public SynthViewV24(Context context, SampleEngine audio) {
        super(context);
        this.audio=audio;
    ...
    private void layoutRects(){
        ...
        kottaButtonRect.set(w*.914f,h*.600f,w*.970f,h*.674f);
        recButtonRect.set(w*.914f,h*.688f,w*.970f,h*.749f);
        playButtonRect.set(w*.914f,h*.761f,w*.970f,h*.822f);

        lessonStripRect.set(w*.215f,h*.135f,w*.785f,h*.306f);
        lessonCloseRect.set(w*.760f,h*.147f,w*.782f,h*.190f);
        ...
        aboutCardRect.set(w*.145f,h*.105f,w*.855f,h*.875f);
        closeAboutRect.set(w*.415f,h*.798f,w*.585f,h*.852f);
    }
    ...
    private void drawKeyboard(Canvas c){
        ...
        drawSpeakerGrille(c,w*.055f,h*.852f,w*.039f,h*.092f);

        drawKottaButton(c);
        drawRecordButton(c);
        drawPlayButton(c);
        drawSpeakerGrille(c,w*.945f,h*.900f,w*.034f,h*.052f);
    ...
    private void drawRecordButton(Canvas c){
        RectF r=recButtonRect;int base=recording?Color.rgb(188,27,37):Color.rgb(229,53,62);drawSideButtonBase(c,r,base);
        float cx=r.centerX(),cy=r.top+r.height()*.36f,rr=r.height()*.12f;
        if(recording){p.setColor(Color.WHITE);c.drawRoundRect(new RectF(cx-rr,cy-rr,cx+rr,cy+rr),h*.002f,h*.002f,p);}else{p.setColor(Color.WHITE);c.drawCircle(cx,cy,rr,p);}
        buttonLabel(c,r,recording?"STOP":"REC");
    }

    private void drawPlayButton(Canvas c){
        RectF r=playButtonRect;boolean enabled=!recordedNotes.isEmpty();int base=playback?Color.rgb(35,143,73):(enabled?Color.rgb(52,174,91):Color.rgb(128,139,143));drawSideButtonBase(c,r,base);
        float cx=r.centerX(),cy=r.top+r.height()*.36f,s=r.height()*.15f;p.setColor(Color.WHITE);Path tri=new Path();tri.moveTo(cx-s*.65f,cy-s);tri.lineTo(cx+s,cy);tri.lineTo(cx-s*.65f,cy+s);tri.close();c.drawPath(tri,p);
        buttonLabel(c,r,playback?"STOP":"PLAY");
    }

    private void drawSideButtonBase(Canvas c,RectF r,int base){
        p.setColor(Color.argb(70,92,59,5));c.drawRoundRect(new RectF(r.left,r.top+h*.004f,r.right,r.bottom+h*.004f),h*.013f,h*.013f,p);
        p.setShader(new LinearGradient(0,r.top,0,r.bottom,lighten(base,1.13f),base,Shader.TileMode.CLAMP));c.drawRoundRect(r,h*.013f,h*.013f,p);p.setShader(null);
        stroke.setColor(Color.WHITE);stroke.setStrokeWidth(h*.0022f);c.drawRoundRect(new RectF(r.left+h*.002f,r.top+h*.002f,r.right-h*.002f,r.bottom-h*.002f),h*.011f,h*.011f,stroke);
    }

    private void buttonLabel(Canvas c,RectF r,String text){
        p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextAlign(Paint.Align.CENTER);p.setTextSize(h*.0125f);p.setColor(Color.WHITE);c.drawText(text,r.centerX(),r.bottom-r.height()*.08f,p);
    }

    private void drawSpeakerGrille(Canvas c,float cx,float cy,float rw,float rh){
        RectF grille=new RectF(cx-rw/2f,cy-rh/2f,cx+rw/2f,cy+rh/2f);
        // Flat inset mesh: intentionally no bevel, glow or button-like shadow.
        p.setColor(Color.argb(120,124,88,20));c.drawRoundRect(grille,h*.010f,h*.010f,p);
        RectF inner=new RectF(grille.left+h*.004f,grille.top+h*.004f,grille.right-h*.004f,grille.bottom-h*.004f);
        p.setColor(Color.rgb(83,76,60));c.drawRoundRect(inner,h*.008f,h*.008f,p);
        float dotR=h*.0027f;float dx=dotR*3.2f,dy=dotR*3.2f;
        p.setColor(Color.rgb(35,37,36));
        for(float yy=inner.top+dy;yy<inner.bottom-dy*.6f;yy+=dy){
            int row=(int)((yy-inner.top)/dy);float offset=(row%2==0?0:dx*.5f);
            for(float xx=inner.left+dx+offset;xx<inner.right-dx*.6f;xx+=dx)c.drawCircle(xx,yy,dotR,p);
        }
    }
    ...
    private void drawLessonStrip(Canvas c){
        ...
        p.setColor(Color.rgb(70,82,95));p.setTextSize(h*.022f);c.drawText("×",lessonCloseRect.centerX(),lessonCloseRect.centerY()-(p.ascent()+p.descent())/2f,p);
        int[] song=SONGS[selectedSong];
        if(lessonPos>=song.length){
            p.setColor(Color.rgb(45,164,82));p.setTextSize(h*.038f);c.drawText("Szuper! Megcsináltad!",lessonStripRect.centerX(),lessonStripRect.centerY()+h*.022f,p);
            p.setColor(Color.rgb(90,90,90));p.setTextSize(h*.017f);c.drawText("A KOTTA gombbal választhatsz új dallamot.",lessonStripRect.centerX(),lessonStripRect.bottom-h*.018f,p);return;
        }
    ...
    private void drawAboutPopup(Canvas c){
        p.setColor(Color.argb(155,0,0,0));c.drawRect(0,0,w,h,p);p.setColor(Color.rgb(255,253,245));c.drawRoundRect(aboutCardRect,h*.040f,h*.040f,p);
        stroke.setColor(Color.rgb(255,194,37));stroke.setStrokeWidth(h*.006f);c.drawRoundRect(new RectF(aboutCardRect.left+h*.004f,aboutCardRect.top+h*.004f,aboutCardRect.right-h*.004f,aboutCardRect.bottom-h*.004f),h*.036f,h*.036f,stroke);
        p.setColor(Color.rgb(32,92,157));p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextAlign(Paint.Align.CENTER);p.setTextSize(h*.047f);c.drawText("Rólunk",aboutCardRect.centerX(),aboutCardRect.top+h*.073f,p);
        p.setTypeface(Typeface.DEFAULT);p.setTextAlign(Paint.Align.LEFT);p.setTextSize(h*.0215f);p.setColor(Color.rgb(55,55,55));
        float x=aboutCardRect.left+h*.064f,y=aboutCardRect.top+h*.118f,maxW=aboutCardRect.width()-h*.128f;
        y=drawWrappedTextY(c,"A Mazsola Klub magyar gyerekdalokkal és játékos zenei tartalmakkal segít megszerettetni a zenét. A Szinti mobilon és tableten is egyszerűen használható.",x,y,maxW,h*.031f)+h*.020f;
        p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextSize(h*.025f);p.setColor(Color.rgb(37,102,163));c.drawText("Hogyan működik?",x,y,p);y+=h*.041f;
        y=drawHelpLine(c,"HANGSZEREK","Felül válassz hangszert, majd játssz a színes és számozott billentyűkön.",x,y,maxW);
        y=drawHelpLine(c,"KOTTÁK","Válassz egy dalt. A pirossal kiemelt billentyű mutatja, melyik hang következik.",x,y,maxW);
        y=drawHelpLine(c,"METRONÓM","Egyenletes kattogással segít tartani a ritmust.",x,y,maxW);
        y=drawHelpLine(c,"TEREM","Bekapcsolva finom koncertterem-visszhangot ad a hangszernek.",x,y,maxW);
        drawHelpLine(c,"FELVÉTEL","Nyomd meg a REC gombot, játssz, majd STOP. A PLAY gombbal visszahallgathatod.",x,y,maxW);
        p.setColor(Color.rgb(43,143,221));c.drawRoundRect(closeAboutRect,h*.023f,h*.023f,p);p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextAlign(Paint.Align.CENTER);p.setTextSize(h*.027f);p.setColor(Color.WHITE);c.drawText("Rendben",closeAboutRect.centerX(),closeAboutRect.centerY()-(p.ascent()+p.descent())/2f,p);
    }

    private float drawHelpLine(Canvas c,String title,String body,float x,float y,float maxW){
        p.setTypeface(Typeface.DEFAULT_BOLD);p.setTextSize(h*.019f);p.setColor(Color.rgb(225,72,53));p.setTextAlign(Paint.Align.LEFT);c.drawText(title,x,y,p);
        float titleW=p.measureText(title)+h*.020f;p.setTypeface(Typeface.DEFAULT);p.setColor(Color.rgb(55,55,55));p.setTextSize(h*.019f);
        String first=body;
        if(titleW+p.measureText(first)<=maxW){c.drawText(first,x+titleW,y,p);return y+h*.037f;}
        return drawWrappedTextY(c,body,x,y+h*.025f,maxW,h*.027f)+h*.015f;
    }
    ...
    private void startLesson(int song){selectedSong=Math.max(0,Math.min(SONGS.length-1,song));lessonPos=0;lessonActive=true;songMenuVisible=false;wrongUntil=0L;invalidate();}

    private void evaluateLesson(Hit hit){
        if(!lessonActive||lessonPos>=SONGS[selectedSong].length)return;int expected=SONGS[selectedSong][lessonPos];
        if(!hit.black&&hit.index==expected){lessonPos++;wrongUntil=0L;}else wrongUntil=SystemClock.uptimeMillis()+400L;
    }
